...
library StrategyCalculator {
    ...
    function validatePercentages(uint256[] memory percentages) internal pure returns (bool isValid) {
    ...
    function getTotalPercentage(uint256[] memory percentages) internal pure returns (uint256 total) {
        for (uint256 i = 0; i < percentages.length; i++) {
