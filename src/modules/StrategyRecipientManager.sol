...
contract StrategyRecipientManager is Ownable {
    ...
    function addRecipient(address recipient, uint256 percentage, string memory label) external onlyOwner {
        ...
        recipients.push(Recipient({addr: recipient, percentage: percentage, isActive: true, label: label}));
    ...
    function updatePercentage(address recipient, uint256 newPercentage) external onlyOwner {
    ...
    function updateLabel(address recipient, string memory newLabel) external onlyOwner {
        uint256 index = recipientIndex[recipient];
    ...
    function batchUpdateRecipients(
        ...
        for (uint256 i = 0; i < newRecipients.length; i++) {
            recipients.push(
                Recipient({addr: newRecipients[i], percentage: newPercentages[i], isActive: true, label: labels[i]})
            );
    ...
    function getActiveRecipients() external view returns (Recipient[] memory activeRecipients) {
